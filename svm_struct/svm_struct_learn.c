...
#include "../svm_extra/svm_struct_api.h"
#include <assert.h>
  ...
  do { /* iteratively increase precision */
	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) {
	    ...
	    /* Give value to ybar! */
	    /* Here they generate the ybar with its loss value. */
			    ...
			    STRUCTMODEL *sm, int alg_type)
{
  ...
  CONSTSET    cset; /* CONSTSET? what is this. */
  SVECTOR     *diff=NULL;
  double      *lhs_n=NULL;
  ...
  */
  ...
  if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
    ...
    /* NOTE:  */
    for(i=0;i<n;i++)
        /* 2014/09/26 Loss function part here: "loss". We only need to change this, I suppose. */
