...
/* 2014/09/19 Model IO part: write_model */
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  /* There's multiple support vector? */
  if(0 && model->kernel_parm.kernel_type == LINEAR) {
    if(verbosity>=1) {
      printf("(compacting..."); fflush(stdout);
    }
    /* Call function to make it compact */
    /* Final output should be a decision fuction. */
...
/* 2014/09/19 It's here. my_malloc. */
