...
/* 2014/09/19 The main classification function?  */
...
/* marker: 2014/09/17   Ranking SVM here*/
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  /* What is this totpair? total pair of possible inversion pair? */
  /* It is. If two document share same queryID and have different ranking order value, totpair++. */
  ...
  /* 2014/09/17  optimization function called here. */
...
/* 2014/09/17 optimization to convergence. Mark. */
     ...
     /* maxdiff: returns maximum violation of KT-conditions */
{
  ...
  for(;retrain && (!terminate);iteration++) {
    ...
    /* The following computation of the objective function works only */
    /* relative to the active variables */
    /* 2014/09/17 objective function here. This is what we need to alter... */
    if(verbosity>=3) {
      criterion=compute_objective_function(a,lin,c,learn_parm->eps,label,
		                           active2dnum);
      printf("Objective function (over active variables): %.16f\n",criterion);
...
/* 2014/09/17 objective function part. */
